...
import { LoaiMonAnStore } from './store/loai-mon-an.store';
 import { AddoreditLoaiMAComponent } from './addoredit/addoreditLoaiMA.component';
...
export class LoaimonanComponent implements OnInit {
  ...
  openAddPopup(): void {
      const dialogRef = this.dialog.open(AddoreditLoaiMAComponent, {
        width: '400px',
      ...
      dialogRef.afterClosed().subscribe((result) => {
        if (result) {
          this.loaimonanService.addLoaiMonAn(result).subscribe(
          {
            next: (res: any) => {
              if (res.data) {
    ...
    openEditPopup(item: any): void {
      ...
      dialogRef.afterClosed().subscribe((result) => {
        if (result) {
        this.loaimonanService.updateLoaiMonAn(item.id, result).subscribe(
          {
            next: (res: any) => {
              if(res.data){
                this.search();
